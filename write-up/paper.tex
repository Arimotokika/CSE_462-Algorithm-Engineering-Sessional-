...
of those targets.  It is NP-complete, and practical instances arising in air
defence and ground-combat scenarios can have hundreds of weapons and targets,
rendering exact methods infeasible within real-time constraints.  This paper
presents two algorithm engineering contributions.  First, we augment the
classic Maximum Marginal Return (MMR) greedy heuristic with (i) tie-breaking
by target value, (ii) a 1-opt iterative local-search pass, and (iii) a 2-opt
swap-search pass, yielding the MMR-IR algorithm.  Second, we redesign the
...
Hybrid GA changes GA performance by $-2.04\%$, $28.85\%$, and $37.70\%$ in the
...
Real-world systems such as Israel's Iron Dome require near-optimal WTA decisions
within seconds: incoming rockets must be assigned to interceptors before they
reach their targets \cite{kline2020}.  This tension between solution quality and
runtime is the core algorithmic challenge.

WTA belongs to the class of NP-complete problems \cite{ahuja2007,lloyd1986}, so practical
solvers rely on heuristics and metaheuristics.  Two widely studied baseline
approaches are the Maximum Marginal Return (MMR) greedy heuristic and a
Genetic Algorithm (GA) \cite{ahuja2007,kline2020}.  MMR is extremely fast but
commits irrevocably to early greedy choices.  Standard GA explores the solution
space more broadly but suffers from slow convergence, solution loss between
generations, and uninformed mutation in large-dimensional instances.
...
\begin{itemize}
  ...
  \item We propose \textbf{Hybrid GA}, which addresses three identified
        ...
        with stagnation detection, and a novel threat-proportional mutation
        operator.
...
Section~\ref{sec:problem} formalises the WTA problem and proves its
NP-completeness.
...
\subsection{NP-Completeness of WTA}
\label{sec:npc}

We establish NP-completeness via two polynomial-time reductions.
The primary formal citation is the classical conference result
\cite{lloyd1986}; we also reviewed a worked pedagogical subset-product
construction in \cite{nearly42np}.
...
Algorithms for WTA fall into five broad categories:
...
Piecewise linear approximation of the survival function achieves $O(mnB)$
complexity \cite{camm2002,andersen2022}.  LP relaxation followed by rounding
yields polynomial complexity but no tight approximation ratio is known.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llll}
    \toprule
    Category      & Complexity          & Max Scale      & Use Case \\
    \midrule
    Exact         & $O(2^{mn})$         & $< 1000$ vars  & Global optimality \\
    Approximation & $O(\mathrm{poly})$  & $> 10000$      & Theoretical bounds \\
    Randomised    & $O(S \cdot mn)$     & Any            & Baseline \\
    Heuristic     & $O(mn)$--$O(m^2n^2)$ & $> 1000$    & Real-time \\
    Metaheuristic & $O(I \cdot P \cdot mn)$ & $> 10000$ & Large-scale \\
    \bottomrule
  \end{tabular}
\end{table}
...
in $O(1)$ per trial instead of $O(WT)$:
\[
...
solution, which is near-optimal in $O(W^2T)$ time.  The remaining individuals
...
\subsubsection{Fix 2: Elitism with Stagnation Detection}
...
\subsubsection{Fix 3: Threat-Proportional Mutation (Novel WTA-Specific Operator)}
...
well-covered targets.  This is a novel, WTA-specific contribution: it
embeds domain knowledge directly into the search operator.
...
Existing WTA benchmark datasets are scarce in the literature and rarely cover
all three size tiers simultaneously; our design therefore deliberately
fills this gap.
...
\begin{enumerate}
  \item \textbf{MMR (original)}: Maximum Marginal Return greedy heuristic,
        ...
        and the algorithm description in \cite{hasan2020}.
  ...
  \item \textbf{Hybrid GA (modified)}: GA with MMR seed, 10\% elitism,
        stagnation detection (threshold $= 5$ generations), and
        threat-proportional mutation; same time budget as GA.
\end{enumerate}

The time budget is set proportionally to problem size.
...
(original) has lower mean $Z$ than Hybrid GA (164.70 vs.\ 168.00), but this
relation reverses strongly in medium and large tiers (906.80 vs.\ 644.51, and
2369.05 vs.\ 1474.28).  Figure~\ref{fig:gap} confirms that GA (original) has
...
\begin{table}[htbp]
  ...
  \begin{tabular}{lccc}
    ...
    Hybrid GA vs.\ GA & -2.04 & 28.85  & 37.70 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Additional Results: Scenario and Scaling Diagnostics}
...
$\max(375, 250) = 375$.  Within a 60-second budget, this yields only
$\approx$160 generations from a purely random start.  Combined with the
...
stagnation injection prevents premature convergence;
...
\begin{itemize}
  ...
  \item The three non-significant categories are all small-tier:
        small\_balanced ($p=0.9734$), small\_scarce ($p=0.9978$),
        and small\_rich ($p=0.1387$).
\end{itemize}
...
modifications that provably improve performance.
...
mutation -- through MMR seeding, elitism with stagnation detection, and the
novel threat-proportional mutation operator.  On balanced tiers, it changes
GA performance by -2.04\%, 28.85\%, and 37.70\% (small/medium/large).  The
...
\bibitem{hasan2020} M.~B. Hasan and Y.~Barua, ``Weapon target assignment,'' in \textit{Concepts, Applications and Emerging Opportunities in Industrial Engineering}, G.~Moynihan, Ed. London, U.K.: IntechOpen, 2020, doi: 10.5772/intechopen.93665.
\bibitem{nearly42np} ``Weapon-target assignment problem,'' \textit{nearly42.org}, Dec.~14, 2013. [Online]. Available: https://www.nearly42.org/cstheory/weapon-target-assignment-problem/ (accessed: Apr.~11, 2026).
...
\bibitem{camm2002} J.~D. Camm, S.~Polasky, A.~Solow, and B.~Csuti, ``A note on optimal algorithms for reserve site selection,'' \textit{Operations Research}, vol.~50, no.~3, pp.~495--501, 2002.
