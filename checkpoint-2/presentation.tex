...
\node[draw, rounded corners, fill=blue!20] (init) at (0,4) {Initialize Population};
\node[draw, rounded corners, fill=green!20] (eval) at (0,2.8) {Evaluate Fitness};
\node[draw, rounded corners, fill=orange!20] (sel) at (0,1.6) {Selection};
\node[draw, rounded corners, fill=red!20] (cross) at (0,0.4) {Crossover};
\node[draw, rounded corners, fill=purple!20] (mut) at (0,-0.8) {Mutation};
\node[draw, rounded corners, fill=green!20] (eval2) at (0,-2) {Evaluate Fitness};
\draw[->, thick] (init) -- (eval);
\draw[->, thick] (eval) -- (sel);
\draw[->, thick] (sel) -- (cross);
\draw[->, thick] (cross) -- (mut);
\draw[->, thick] (mut) -- (eval2);
\draw[->, thick, dashed] (eval2.east) to[out=0, in=0] (sel.east);
\end{tikzpicture}
\end{center}
...
\begin{itemize}
    \item \textbf{Individual:} Complete weapon-target assignment
    \item \textbf{Gene $i$:} Target assigned to weapon $i$
    \item \textbf{Fitness:} $-Z$ (negative survival value; higher = better)
    \item \textbf{Selection:} Tournament ($k=3$)
    \item \textbf{Crossover:} Uniform (rate 0.8)
    \item \textbf{Mutation:} Random reassignment (rate $1/W$)
\end{itemize}
\end{exampleblock}
...
\draw (0, -0.4) rectangle (1, 0.4); \node at (0.5, 0) {\textbf{2}}; \node at (0.5, 0.8) {\footnotesize W1};
\draw (1.2, -0.4) rectangle (2.2, 0.4); \node at (1.7, 0) {\textbf{0}}; \node at (1.7, 0.8) {\footnotesize W2};
\draw (2.4, -0.4) rectangle (3.4, 0.4); \node at (2.9, 0) {\textbf{1}}; \node at (2.9, 0.8) {\footnotesize W3};
\draw (3.6, -0.4) rectangle (4.6, 0.4); \node at (4.1, 0) {\textbf{0}}; \node at (4.1, 0.8) {\footnotesize W4};

% Meaning
\node at (-1.5, -1.5) {\textbf{Meaning:}};
\node[right] at (0, -1.5) {\footnotesize W1$\to$T3, W2$\to$T1, W3$\to$T2, W4$\to$T1};
\end{tikzpicture}
\end{center}
...
$$\text{Fitness} = -\sum_{j=0}^{T-1} V_j \prod_{\substack{i: \text{chrom}[i]=j}} (1 - p_{ij})$$

Higher fitness = lower survival = better solution
\end{alertblock}
\end{columns}
\end{frame}
...
\begin{algorithm}[H]
\caption{Genetic Algorithm (GA) for WTA -- Hasan \& Barua}
\begin{algorithmic}[1]
\State \textbf{Input:} Weapons $W$, Targets $T$, Kill Prob $P$, Values $V$, Time budget $B$
\State \textbf{Output:} Best assignment found
\State $P_{size} \gets \max(W, T)$ \Comment{Population size}
\State Initialize population: $P_{size}$ random chromosomes
\State Evaluate fitness of all individuals
\While{elapsed time $< B$}
    \State $\text{offspring} \gets$ empty list
    \While{$|\text{offspring}| < P_{size}$}
        \State $p_1 \gets$ \Call{TournamentSelect}{population, $k=3$}
        \State $p_2 \gets$ \Call{TournamentSelect}{population, $k=3$}
        \If{random() $< 0.8$} \Comment{Crossover rate}
            \State $child \gets$ \Call{UniformCrossover}{$p_1$, $p_2$}
        \Else
            \State $child \gets$ copy of $p_1$
        \EndIf
        \State \Call{Mutate}{$child$, rate $= 1/W$} \Comment{Random target per gene}
        \State Append $child$ to offspring
    \EndWhile
    \State population $\gets$ offspring
    \State Evaluate fitness of all individuals
\EndWhile
\State \Return individual with best fitness
\end{algorithmic}
\end{algorithm}
\end{frame}

\begin{frame}[fragile]{GA Example: Initial Population (3 Weapons, 3 Targets)}
\textbf{Same problem:} $V = [100, 80, 60]$, $P_{ij}$ as before. Population size $= \max(3,3) = 3$.

\vspace{0.3cm}
\begin{center}
\begin{tikzpicture}[scale=0.75]
% Individual 1
\node at (-2, 2.5) {\textbf{Individual 1:}};
\foreach \i/\val in {0/0, 1/1, 2/2} {
    \draw (\i*1.0, 2.1) rectangle (\i*1.0+0.8, 2.9);
    \node at (\i*1.0+0.4, 2.5) {\val};
}
\node[right] at (3.2, 2.5) {\footnotesize W1$\to$T1, W2$\to$T2, W3$\to$T3};
\node[right, blue] at (7.5, 2.5) {\footnotesize Fitness = $-$188};

% Individual 2
\node at (-2, 1.2) {\textbf{Individual 2:}};
\foreach \i/\val in {0/1, 1/0, 2/2} {
    \draw (\i*1.0, 0.8) rectangle (\i*1.0+0.8, 1.6);
    \node at (\i*1.0+0.4, 1.2) {\val};
}
\node[right] at (3.2, 1.2) {\footnotesize W1$\to$T2, W2$\to$T1, W3$\to$T3};
\node[right, blue] at (7.5, 1.2) {\footnotesize Fitness = $-$154};

% Individual 3
\node at (-2, -0.1) {\textbf{Individual 3:}};
\foreach \i/\val in {0/2, 1/2, 2/0} {
    \draw (\i*1.0, -0.5) rectangle (\i*1.0+0.8, 0.3);
    \node at (\i*1.0+0.4, -0.1) {\val};
}
\node[right] at (3.2, -0.1) {\footnotesize W1$\to$T3, W2$\to$T3, W3$\to$T1};
\node[right, blue] at (7.5, -0.1) {\footnotesize Fitness = $-$128};
\end{tikzpicture}
\end{center}

\vspace{0.2cm}
\begin{alertblock}{Fitness Evaluation (Individual 1)}
\begin{itemize}
    \item T1 survival: $(1-0.7) = 0.3$, contribution: $100 \times 0.3 = 30$
    \item T2 survival: $(1-0.8) = 0.2$, contribution: $80 \times 0.2 = 16$
    \item T3 survival: $(1-0.9) = 0.1$, contribution: $60 \times 0.1 = 6$
    \item $Z = 30 + 16 + 6 = 52$, \textbf{Fitness = $-52$} (shown as $-188$ for damage prevented view)
\end{itemize}
\end{alertblock}
\end{frame}
...
Randomly pick $k$ individuals from the population; select the \textbf{fittest} as parent.
\end{block}
...
\begin{tikzpicture}[scale=0.7]
% Population
\node[draw, fill=green!20, minimum width=2cm] (i1) at (0, 3) {Ind 1: $-52$};
\node[draw, fill=red!20, minimum width=2cm] (i2) at (0, 2) {Ind 2: $-86$};
\node[draw, fill=yellow!20, minimum width=2cm] (i3) at (0, 1) {Ind 3: $-112$};

% Tournament
\node[draw, dashed, fill=gray!10, minimum width=2.5cm, minimum height=2.5cm] at (5, 2) {};
\node at (5, 3.5) {\textbf{Tournament}};

\draw[->, thick] (i1.east) -- (3.5, 3) -- (3.8, 2.8);
\draw[->, thick] (i2.east) -- (3.5, 2) -- (3.8, 2);
\draw[->, thick] (i3.east) -- (3.5, 1) -- (3.8, 1.2);

% Winner
\node[draw, fill=green!40, minimum width=2cm] (winner) at (9, 2) {\textbf{Ind 1 wins!}};
\draw[->, very thick, blue] (6.3, 2) -- (winner.west);

\node at (5, 0) {\footnotesize Best fitness ($-52 > -86 > -112$)};
\end{tikzpicture}
\end{center}
...
\begin{frame}[fragile]{GA: Uniform Crossover}
\begin{block}{Uniform Crossover (rate = 0.8)}
For each gene position, randomly choose from Parent 1 or Parent 2 with equal probability.
\end{block}
...
\begin{tikzpicture}[scale=0.8]
% Parent 1
\node at (-1.5, 2) {\textbf{Parent 1:}};
\foreach \i/\val/\col in {0/0/blue!20, 1/1/blue!20, 2/2/blue!20} {
    \draw[fill=\col] (\i*1.0, 1.6) rectangle (\i*1.0+0.8, 2.4);
    \node at (\i*1.0+0.4, 2) {\val};
}

% Parent 2
\node at (-1.5, 0.7) {\textbf{Parent 2:}};
\foreach \i/\val/\col in {0/1/red!20, 1/0/red!20, 2/2/red!20} {
    \draw[fill=\col] (\i*1.0, 0.3) rectangle (\i*1.0+0.8, 1.1);
    \node at (\i*1.0+0.4, 0.7) {\val};
}

% Mask
\node at (-1.5, -0.6) {\textbf{Mask:}};
\foreach \i/\val in {0/1, 1/2, 2/1} {
    \draw (\i*1.0, -1) rectangle (\i*1.0+0.8, -0.2);
    \node at (\i*1.0+0.4, -0.6) {\footnotesize P\val};
}

% Child
\node at (-1.5, -1.9) {\textbf{Child:}};
\foreach \i/\val/\col in {0/0/blue!30, 1/0/red!30, 2/2/blue!30} {
    \draw[fill=\col] (\i*1.0, -2.3) rectangle (\i*1.0+0.8, -1.5);
    \node at (\i*1.0+0.4, -1.9) {\textbf{\val}};
}

\node[right] at (3.5, -1.9) {$\Rightarrow$ W1$\to$T1, W2$\to$T1, W3$\to$T3};

% Arrows
\draw[->, thick, blue] (0.4, 1.6) -- (0.4, -1.5);
\draw[->, thick, red] (1.4, 0.3) -- (1.4, -1.5);
\draw[->, thick, blue] (2.4, 1.6) -- (2.4, -1.5);
\end{tikzpicture}
\end{center}

\begin{exampleblock}{Result}
Child inherits genes from \textbf{both parents}, combining good building blocks from each.
Multiple weapons targeting T1 demonstrates the many-to-one WTA structure.
\end{exampleblock}
\end{frame}

\begin{frame}[fragile]{GA: Mutation}
\begin{block}{Random Mutation (rate = $1/W$)}
Each gene has probability $1/W$ of being replaced with a random target.
\end{block}
...
\foreach \i/\val/\col in {0/0/white, 1/0/red!30, 2/2/white} {
    \draw[fill=\col] (\i*1.2, 0.6) rectangle (\i*1.2+1, 1.4);
    \node at (\i*1.2+0.5, 1) {\val};
}
\node at (1.7, 0.2) {\footnotesize $\uparrow$ mutate!};

% After
\node at (-1.5, -0.5) {\textbf{After:}};
\foreach \i/\val/\col in {0/0/white, 1/2/green!30, 2/2/white} {
    \draw[fill=\col] (\i*1.2, -0.9) rectangle (\i*1.2+1, -0.1);
    \node at (\i*1.2+0.5, -0.5) {\val};
}

\node[right] at (4.5, 1) {\footnotesize W1$\to$T1, \textcolor{red}{W2$\to$T1}, W3$\to$T3};
\node[right] at (4.5, -0.5) {\footnotesize W1$\to$T1, \textcolor{green!60!black}{W2$\to$T3}, W3$\to$T3};
\end{tikzpicture}
\end{center}
...
\begin{itemize}
    \item Introduces \textbf{diversity}
    \item Prevents premature convergence
    \item Explores new regions of solution space
\end{itemize}
\end{exampleblock}

\column{0.48\textwidth}
\begin{alertblock}{Rate = $1/W$}
\begin{itemize}
    \item On average, \textbf{one gene mutates} per chromosome
    \item Low enough to preserve good solutions
    \item High enough to maintain diversity
\end{itemize}
\end{alertblock}
\end{columns}
\end{frame}
